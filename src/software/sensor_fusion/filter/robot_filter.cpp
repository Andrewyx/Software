      ...
      expiry_buffer_duration(expiry_buffer_duration)
{
    ...
    initial_state.x << current_robot_state.position().x(),
        current_robot_state.position().y(), current_robot_state.velocity().x(),
      ...
      expiry_buffer_duration(expiry_buffer_duration)
{
    ...
    initial_state.timestamp = current_robot_state.timestamp;
    initial_state.x         = Eigen::VectorXf::Zero(STATE_DIM);
    initial_state.x << current_robot_state.position.x(), current_robot_state.position.y(),
        0.0, 0.0, current_robot_state.orientation.clamp().toRadians(), 0.0;
    initial_state.P           = Eigen::MatrixXf::Identity(STATE_DIM, STATE_DIM) * 0.1;
...
bool RobotFilter::update(FilterState& state, const RobotDetection& measurement)
{
    ...
    z << measurement.position.x(), measurement.position.y(),
        measurement.orientation.clamp().toRadians();
...
void RobotFilter::pruneBuffer(Timestamp latest_timestamp)
{
    ...
    Timestamp cutoff = Timestamp::fromMilliseconds(latest_timestamp.toMilliseconds() -
                                                   HORIZON_BUFFER_DURATION_MILLISECONDS);

    auto it = horizon_buffer.begin();
    while (it != horizon_buffer.end() && it->timestamp < cutoff &&
           horizon_buffer.size() > 1)
    {
        it++;
    }
    ...
    std::vector<RobotDetection> detections)
{
    ...
    for (size_t i = 1; i < detections.size(); ++i)
    {
        ...
        {
            current_avg.position =
                current_avg.position + detections[i].position.toVector();
    ...
    const std::optional<RobotId> breakbeam_tripped_id)
{
    ...
    for (const auto& detection : relevant_detections)
    {
        ...
        if (!horizon_buffer.empty() &&
            detection.timestamp < horizon_buffer.front().timestamp)
        {
            continue;
        }
        ...
        auto it =
            std::lower_bound(horizon_buffer.begin(), horizon_buffer.end(), detection,
        ...
        {
            // Already have a state here, update measurement if we want, but averaging
            // already handled duplicates. If it's a completely new measurement at the
            // same time, we'll replace it.
    ...
    if (buffer_modified)
    {
        ...
        // rewind_start_idx is guaranteed to be >= 1 if we maintain at least one initial
        // state
        ...
        for (size_t i = start_idx; i < horizon_buffer.size(); ++i)
        {
            ...
            double dt = (horizon_buffer[i].timestamp - horizon_buffer[i - 1].timestamp)
                            .toSeconds();
            ...
            {
                bool accepted =
                    update(horizon_buffer[i], horizon_buffer[i].measurement.value());
                if (!accepted)
                {
                    // If rejected as outlier, clear the measurement so we don't apply it
                    // again on future rewinds
    ...
    // Expiry check
    // If the latest state in our buffer (which is our last known robot state) is older
    // than expiry duration, we consider the robot expired.
    if (!horizon_buffer.empty())
    {
        Timestamp last_robot_timestamp = horizon_buffer.back().timestamp;
        if (current_time.toMilliseconds() > expiry_buffer_duration.toMilliseconds() +
                                                last_robot_timestamp.toMilliseconds())
        {
            return std::nullopt;
        }
        ...
        this->current_robot_state =
            Robot(this->getRobotId(), Point(final_state.x(0), final_state.x(1)),
                  Vector(final_state.x(2), final_state.x(3)),
                  Angle::fromRadians(final_state.x(4)),
                  AngularVelocity::fromRadians(final_state.x(5)), final_state.timestamp,
                  breakbeam_tripped);
