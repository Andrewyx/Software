#include "enemy_kickoff_play_fsm.h"
...
EnemyKickoffPlayFSM::EnemyKickoffPlayFSM(std::shared_ptr<Strategy> strategy)
        : DefensePlayFSMBase(strategy)
{
}

void EnemyKickoffPlayFSM::blockEnemyKicker(const Update& event)
{
    setTactics(event, event.common.num_tactics);
}

void EnemyKickoffPlayFSM::setTactics(const Update& event, unsigned int num_tactics)
{
    if (num_tactics <= 0)
    {
        return;
